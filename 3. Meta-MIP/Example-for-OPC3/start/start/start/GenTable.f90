...
double precision :: r,f(2),g(2),Rc,ep,R0,kp
open(unit=1,file='table.xvg',status='unknown',action='write')
write(*,'(a87)') 'Input parameters: Epsilon,Rmin,Kappa,Rcutoff; and potential type (1=LJ12-6, 2=LJ12-6-4)'
read(*,*) ep,R0,kp,Rc,pt
write(*,'(4f10.5,i2)') ep,R0,kp,Rc,pt
if ( pt .eq. 2 ) then
    ep=ep*4.1858518
    R0=R0*0.200000
    kp=kp*100.000
...
do i=1,nbins
    ...
    if ( pt .eq. 1 ) then
        ...
        f(1) = ep * ( R0**12/r**12 - 2*R0**6/r**6 - 2*kp*R0**6/r**4 )
        f(2) = ep * ( 12*R0**12/r**13 - 12*R0**6/r**7 - 8*kp*R0**6/r**5 )
    else
