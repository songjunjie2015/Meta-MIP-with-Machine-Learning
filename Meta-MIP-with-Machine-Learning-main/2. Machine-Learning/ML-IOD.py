...
def load_data(train_path, pre_path):
    ...
    X_pred = pd.read_csv(pre_path, header=None).iloc[:, :2].values
    Y_pred = pd.read_csv(pre_path, header=None).iloc[:, 2:3].values
...
if __name__ == "__main__":
    ...
    for random_state in range(1, 6):
        ...
        X_train_split, X_val_split, y_train_split, y_val_split = train_test_split(
            X_train, y_train, test_size=0.15, random_state=random_state, shuffle=True
        )
